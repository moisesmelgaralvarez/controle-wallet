-- ============================================================
-- Las compras de una cuenta bajan de esa cuenta, y lo retenido se guarda
--
-- DOS DATOS QUE SE LEÍAN Y NO SE GUARDABAN
--
-- 1. LA COMPRA CON DÉBITO. Un gasto leído del estado de cuenta de una
--    CUENTA se colgaba de la tarjeta de débito ligada a esa cuenta. Si el
--    hogar no la había anotado —lo normal—, quedaba como «tarjeta» sin
--    tarjeta: no bajaba la cuenta ni subía ninguna deuda. Ahora el
--    navegador propone la tarjeta y esta función la crea en la misma
--    transacción que los movimientos que cuelgan de ella.
--
-- 2. LO RETENIDO DE UNA CUENTA. Desde la primera versión, el UPDATE de lo
--    retenido solo corría si el destino era una tarjeta. Pero el único
--    lector que lo encuentra es el de la cuenta («retenidos y diferidos»).
--    Se leía, se enseñaba en pantalla y se tiraba al guardar.
--
-- La firma cambia (un parámetro más), así que se borra la anterior:
-- `create or replace` con otra lista de parámetros crea una SEGUNDA
-- función, y PostgREST no sabría a cuál llamar.
--
-- Reverso: supabase/reversos/20260918120000_debito_y_retenido_de_cuenta.reverso.sql
-- ============================================================
