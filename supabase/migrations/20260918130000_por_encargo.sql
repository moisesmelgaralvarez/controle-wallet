...
-- Compras por encargo
--
-- El dueño: «puede que sea una compra no muy recurrente en la que pude
-- hacerle un favor —camisas deportivas de un hermano, quien después me
-- mandó el dinero de EE. UU. para pagar lo gastado—».
--
-- Esa compra salió de la tarjeta: cuenta en la deuda, en el ciclo y en
-- los saldos. Pero no es gasto de la casa. Si entrara al presupuesto, un
-- favor de L 8,000 haría que Ropa «se pasara» ese mes y, peor, inflaría
-- por un año la media con que se sugiere el plan.
--
-- NO SE ADIVINA. Se pensó en emparejar solo el pago que llega con la
-- compra que se le parece, y se descartó: un pago de L 8,000 se parece a
-- cualquier compra de L 8,000. La marca la pone la persona, que es la
-- única que sabe.
--
-- Y SOBREVIVE A REIMPORTAR. `importar_lote` borra y vuelve a escribir lo
-- importado de ese rango —así no se duplica cuando la semana nueva trae la
-- anterior—, y con eso se habría llevado la marca. Se recuerda antes de
-- borrar y se vuelve a poner después, por fecha y monto.
--
-- Reverso: supabase/reversos/20260918130000_por_encargo.reverso.sql
-- ============================================================

alter table public.movimientos
  add column encargo boolean not null default false;

comment on column public.movimientos.encargo is
  'Comprado para otra persona, que lo devuelve. Cuenta en la tarjeta y en los saldos; no en el gasto de la casa ni en la media.';

-- ------------------------------------------------------------
-- `importar_lote`, igual que en 20260918120000, más la marca que
-- sobrevive. Misma firma: `create or replace` basta.
-- ------------------------------------------------------------
...
begin
  ...
  /* Lo marcado «por encargo» se recuerda ANTES de borrar: el archivo no
     lo trae —eso lo sabe la persona, no el banco— y sin esto cada
     reimportación lo desmarcaba en silencio. */
  select coalesce(jsonb_agg(jsonb_build_object('f', fecha, 'm', monto)), '[]'::jsonb)
    into v_encargos
    from public.movimientos
   where hogar_id = v_hogar and origen = 'import' and fuente = v_fuente
     and fecha between p_desde and p_hasta and encargo;
  ...
  /* Y se vuelve a poner, por fecha y monto: el concepto no sirve para
     emparejar, porque el PDF de la semana y el CSV del mes lo escriben
     distinto. Dos compras del mismo día por el mismo monto quedarían
     marcadas las dos; se desmarca una a mano, y es mucho menos grave que
     perder la marca de todas cada semana. */
  if jsonb_array_length(v_encargos) > 0 then
    update public.movimientos m
       set encargo = true
     where m.hogar_id = v_hogar and m.origen = 'import' and m.fuente = v_fuente
       and m.lote = p_lote and m.fecha between p_desde and p_hasta
       and exists (select 1 from jsonb_array_elements(v_encargos) e
                    where (e->>'f')::date = m.fecha and (e->>'m')::numeric = m.monto);
    get diagnostics v_n = row_count;
    v_res := v_res || jsonb_build_object('encargos_conservados', v_n);
  end if;
...
comment on function public.importar_lote is
  'Aplica un estado de cuenta en UNA transacción: crea la tarjeta de débito de la cuenta si no existía, crea los rubros que el archivo obliga —con el identificador que ya traen sus movimientos—, guarda el aprendizaje comercio → rubro, borra lo importado antes de esa fuente en ese rango —conservando la marca «por encargo»— e inserta lo del archivo. El saldo declarado solo avanza: un archivo viejo trae sus movimientos sin tocar el saldo de hoy. Va security invoker para que RLS siga decidiendo.';
